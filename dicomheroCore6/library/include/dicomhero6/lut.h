...
#ifndef SWIG
protected:
    explicit LUT(const std::shared_ptr<dicomhero::implementation::lut>& pLut);

private:
    friend const std::shared_ptr<dicomhero::implementation::lut>& getLUTImplementation(const LUT& lut);
    std::shared_ptr<dicomhero::implementation::lut> m_pLut;
#endif
};
