...
Each group is represented by its ID, which doesn't have to be unique; for instance,
 if several images are embedded in the dataSet, then several groups with the same
 ID could be present (actually, this happens only with older Dicom streams, since
 the new streams should use the sequences in order to achieve the same result).

The information stored in the dataSet (\ref puntoexe::imebra::dataSet) 
 represent the content of one or more related Dicom objects: for instance it 
...
When a group appears several times in a dataSet, then it is identified also by
 a progressive number (called order in the library), which usually is 0 for the 
 groups that appear only once. The information about the order is not stored in the 
 Dicom stream but is generated by the library while a Dicom structure is created.
...
Each tag is identified by a numeric ID: the tag's ID inside a group 
 (see \ref imebra_structure_groups) must be unique.
