...

        {
            ...
            if(inputHighBit > outputHighBit)
            {
                ...
                for(; inputHeight != 0; --inputHeight)
                {
                    ...
                    {
                        outputValue = (outputType)((((imbxInt32)*(pInputMemory++) - inputHandlerMinValue) >> rightShift) + outputHandlerMinValue);
            ...
            else
            {
                ...
                for(; inputHeight != 0; --inputHeight)
                {
                    for(int scanPixels(inputWidth); scanPixels != 0; --scanPixels)
                    {
                        outputValue = (outputType)((((imbxInt32)*(pInputMemory++) - inputHandlerMinValue) << leftShift) + outputHandlerMinValue);
                        *pOutputMemory = outputValue;
                        *++pOutputMemory = outputValue;
                        *++pOutputMemory = outputValue;
                        ++pOutputMemory;
